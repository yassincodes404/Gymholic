...
export default function AdminProductsPage() {
  ...
  async function softDeleteProduct(product: StoreProductRow) {
    setBusy(true);
    try {
      await adminFetch(`store/admin/products/${product.id}`, { method: "DELETE" });
      flash("notice", "Product removed from the store.");
      await load();
    } catch (err) {
      flash("error", err instanceof Error ? err.message : "Could not remove the product.");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <AdminShell activeHref="/admin/products">
      ...
      {/* Categories */}
      <div className="bg-surface border border-paper/10 rounded-xl p-5 mb-8">
        ...
        <form onSubmit={createCategory} className="flex gap-3 max-w-md">
          ...
          <button
            ...
            className="bg-orange text-void font-semibold text-sm px-5 py-2 rounded-full hover:bg-orange/90 transition-colors disabled:opacity-50"
          >
            Add
          </button>
        </form>
      ...
      <div className="bg-surface border border-paper/10 rounded-xl p-5 mb-8">
        <div className="flex items-center justify-between mb-4">
          ...
          <button
            type="button"
            onClick={startCreate}
            className="bg-orange text-void font-semibold text-sm px-5 py-2 rounded-full hover:bg-orange/90 transition-colors"
          >
            New Product
          </button>
        </div>
                ...
                {products.map((product) => (
                  <tr key={product.id} className="border-b border-paper/5">
                    ...
                    <td className="py-2.5 text-right whitespace-nowrap">
                      <button type="button" onClick={() => startEdit(product)} className="text-xs text-paper/70 hover:text-paper mr-3">
                        Edit
                      </button>
                      <button
                        type="button"
      ...
      {/* Create / edit form */}
      <div ref={formRef} className="bg-surface border border-paper/10 rounded-xl p-5">
        ...
        <form onSubmit={saveProduct} className="grid gap-4 md:grid-cols-2 max-w-3xl">
          ...
          <div className="md:col-span-2 flex gap-3">
            <button
              type="submit"
              disabled={busy}
              className="bg-orange text-void font-semibold text-sm px-6 py-2.5 rounded-full hover:bg-orange/90 transition-colors disabled:opacity-50"
            >
